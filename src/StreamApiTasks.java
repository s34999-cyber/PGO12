...
public class StreamApiTasks {
    ...
    static List<String> activeOrderIds(List<Order> orders) {
        return orders.stream()
                .filter(Order -> !Order.status().equals(OrderStatus.CANCELLED))
                .map(Order::id)
                .toList();
    ...
    static List<Order> ordersAbove(List<Order> orders, double minValue) {
        return orders.stream().filter(Order -> Order.totalValue() > minValue).toList();
    }

    static List<String> uniqueCustomerNames(List<Order> orders) {
        return orders.stream().map(Order::customerName).distinct().sorted().toList();
    }

    static List<String> soldProductNames(List<Order> orders) {
        return orders.stream().filter(Order -> !Order.status().
                equals(OrderStatus.CANCELLED)).flatMap(Order -> Order.items().stream())
                .map(item -> item.product().name()).distinct().sorted().toList();
    ...
    static Map<String,Double> topCustomers(List<Order> orders, int limit) {
        ...
        top3.entrySet().stream().limit(limit);
        return top3;
    }
    ...
    static Optional<Order> mostExpensiveDeliveredOrder(List<Order> orders) {
        ...
        return orders.stream().filter(Order -> Order.status().equals(OrderStatus.DELIVERED))
